        ...
        for epoch in range(num_epochs := 40):
            ...
            for idx, (images, masks) in tqdm(enumerate(train_loader), total=len(train_loader)):
                ...
                if epoch < 3:  # won't be used
                    ...
                    elif config.loss_type == 'complete_iou':
...
sweep_config = {'method': 'random',
                'name': 'sweep_1',
                'program': 'train_bbox.py',
                'metric': {
                    'goal': 'minimize',
                    'name': 'val_loss'
                    },
                'parameters': {
                    'batch_size': {'min': 1, 'max': 64, 'distribution': 'int_uniform'},
                    'lr': {'max': 0.00001, 'min':  0.0000001},
                    'base_dim': {'values': [32, 64, 80]},
                    'dropout': {'values': [0.1, 0.2, 0.3, 0.4]},
                    'batch_norm': {'values': [True, False]},
                    'loss_type':{'values': ['general_iou', 'complete_iou', 'mse'], 'probabilities': [0.3, 0.4, 0.3]}
                    },
